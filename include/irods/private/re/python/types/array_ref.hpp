...
namespace {
    ...
    template<typename T> using BaseType = typename base_type<T>::type;
    ...
    template<typename T> using PointerType = typename pointer_type<T>::type;
...
class array_ref : public array_ref_base<T>
{
    public:
        ...
        template<bool b = NullTerminated, typename = typename std::enable_if_t<b>>
        ...
        array_ref( pointer arr, const std::vector<size_type>& lengths, std::size_t size_index = 0 )
            : begin_{arr}
            , length_{lengths[size_index]}
            {}
...
//          *    Number of elements pointed at
//          */
//         constexpr null_terminated_array_ref( pointer arr, size_type length ) : array_ref<T>{arr, length} {}
// 
//         /**
//          * Constructor that accepts a reference to an array
//          *
...
//          */
//         template<size_type N>
//             constexpr null_terminated_array_ref( T (&arr)[N] ) noexcept : array_ref<T>{arr} {}
// 
//         /**
//          * Constructor taking a pair of pointers pointing to the first element and one past the last
//          * element of the array, respectively.
...
//          *    Pointer to one past the last element of the array
//          */
//         null_terminated_array_ref( pointer first, pointer last ) noexcept : array_ref<T>{first, last} {}
// 
//         /** Copy constructor */
//         constexpr null_terminated_array_ref( null_terminated_array_ref const& ) noexcept = default;
// 
//         /** Copy assignment operator */
//         null_terminated_array_ref& operator=( null_terminated_array_ref const& ) noexcept = default;
// 
//         /** Move constructor */
//         constexpr null_terminated_array_ref( null_terminated_array_ref&& ) noexcept = default;
// 
//         /** Move assignment operator */
//         null_terminated_array_ref& operator=( null_terminated_array_ref&& ) noexcept = default;
// 
// };
...
class array_ref<array_ref<T, b>> : public array_ref_base<array_ref<T, b>>
{
    ...
    public:
        ...
        /** Alias for a reverse iterator pointing at value_type objects */
        using reverse_iterator = typename std::vector<array_ref<T, b>>::reverse_iterator;
        /** Alias for a constant reverse iterator pointing at value_type objects */
        using const_reverse_iterator = typename std::vector<array_ref<T, b>>::const_reverse_iterator;
        ...
        array_ref( internal_pointer arr, const std::vector<size_type>& lengths, std::size_t size_index = 0 )
        {
